...
class PostTest(TestCase):
    ...
    # 게시글 전체 목록
    print(Post.objects.all().values_list('post_title'))
    # 게시글 1개 조회
    print(Post.objects.filter(id=55).values('post_title','post_content'))
    # 해당 게시글의 댓글 목록 조회
    print(Post.objects.filter(id=55).values('reply__reply_content'))
    # print(post.reply_set.all())
    # 게시글과 게시글 작성자 정보 한 번에 조회
    print(Post.objects.filter(id=55).values('post_title','post_content','member__member_name','member__member_email','member__member_age'))
    pass
