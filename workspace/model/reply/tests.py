...
class ReplyTest(TestCase):
    ...
    # 게시글 상세보기 (작성자 필요)
    print(Post.objects.filter(id=54).annotate(member_name=F('member__member_name'))
          .values('id', 'post_title', 'post_content', 'member_name'))
    # 댓글 목록 (게시글 제목, 작성자 필요)
    print(Post.objects.filter(id=54).annotate(member_name=F('member__member_name'))
          .values('post_title', 'reply__reply_content', 'reply__member__member_name'))
    # 특정 댓글의 대댓글 목록 (댓글 내용, 작성자 필요)
    print(Reply.objects.filter(Q(id=3) | Q(reply_group_id=3,reply_depth=2)).annotate(member_name=F('member__member_name'))
          .values('reply_content', 'post__reply__member'))
    pass
